...
5. 预览在屏外窗维护单张带 64 像素安全边界的 DWM 缩略图。边界内移动只从复用缓冲中做 CPU 对齐裁剪，不改变 DWM 来源；越过边界才重定位缩略图。`PrintWindow` 完成后重新读取最新鼠标位置，在同一捕获缓冲内做提交前对齐。最终尺寸抓帧套用启用时预计算的形状 alpha；显示窗在 F8 会话内复用 DIB、DC 和像素缓冲，再用 `UpdateLayeredWindow` 一次提交内容与屏幕位置。Keep one off-screen DWM thumbnail with a 64 px safety margin. Motion inside the margin uses CPU-aligned cropping from a reused buffer without changing the DWM source; only crossing the margin repositions the thumbnail. After `PrintWindow`, read the latest pointer position and realign inside the same captured buffer before presentation. Apply the precomputed shape alpha to the final-sized crop; reuse the DIB, DCs, and pixel buffer for the F8 session, then submit content and screen position with one `UpdateLayeredWindow` call.
...
圆形和圆角矩形都由分层位图 alpha 蒙版保证。`--visual-smoke` 会覆盖静止刷新、真实鼠标移动、后台 Hover 重绘、内容坐标对齐、提交前鼠标延迟锁定、换帧预算、羽化 alpha、黑帧和形状回归。个别来源窗口若 DWM 缩略图/PrintWindow 抓取失败，预览可能不可用。更激进的合成方式不在本页承诺范围内。

Both circles and rounded rectangles are guaranteed by layered bitmap alpha. `--visual-smoke` covers stationary refresh, real-pointer motion, background hover repaint, content alignment, pre-present pointer late-latching, frame-budget metrics, feather alpha, black frames, and shape regressions. Some source windows may fail DWM thumbnail/PrintWindow capture. More aggressive compositing approaches are outside this document’s promises.
