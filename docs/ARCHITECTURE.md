...
5. 全屏三角形 pixel shader 同时执行一比一采样、圆形/圆角矩形 signed-distance mask 和羽化预乘 alpha。两缓冲 BGRA 交换链比实际透视形状四周各大 192 像素；鼠标在安全余量内移动时，顶层 HWND 保持固定，只在同一次 shader/Present 中改变采样坐标和形状中心。跨出安全余量时，先隐藏并移动 HWND，清透明交换链后再显示目标帧，避免新帧在旧坐标被合成一次。A fullscreen-triangle pixel shader performs one-to-one sampling, circle/rounded-rectangle signed-distance masking, feathering, and premultiplied alpha. The two-buffer BGRA swap chain has a 192 px margin around the portal shape. While the pointer stays inside that margin, the top-level HWND remains fixed and only the sampling coordinates and shape center change in the same shader/Present. On crossing the margin, the HWND is hidden and moved first, then the transparent-cleared destination frame is shown so a new frame cannot be composited once at the old coordinates.
...
GPU 形状由 HLSL signed-distance mask 保证，CPU 回退形状由分层位图 alpha 蒙版保证。`--gpu-probe` 验证 WGC/D3D11/DirectComposition/HLSL，`--gpu-portal-smoke-hwnd` 验证常驻纹理、高精度移动裁剪、输入命中透明与安全画布重定位次数，`--visual-smoke` 继续覆盖 CPU 管线的黑帧、形状和坐标回归。受保护内容、某些游戏/反作弊窗口、无重定向表面或驱动限制可能让 WGC 只返回空白/静态帧；这些情况不通过注入、驱动或绕过保护来解决。

GPU shapes are produced by HLSL signed-distance masks, while CPU fallback shapes use layered-bitmap alpha masks. `--gpu-probe` verifies WGC/D3D11/DirectComposition/HLSL, `--gpu-portal-smoke-hwnd` covers persistent textures, high-resolution motion cropping, transparent input hit-testing, and safe-canvas relocation counts, and `--visual-smoke` retains CPU black-frame, shape, and alignment regressions. Protected content, some game/anti-cheat windows, non-redirected surfaces, or driver restrictions may make WGC return blank or static frames; PierceView does not use injection, drivers, or protection bypasses to work around them.
