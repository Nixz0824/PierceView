...
寸镜不转发或合成鼠标事件。跟随鼠标中心的小型交互孔让 Windows 自己把真实鼠标事件送给下面的窗口；完整圆形或矩形只属于视觉层。GPU 视觉窗使用系统级分层透明命中跳过，宿主交互孔移动后重绘刚恢复覆盖的旧位置。透视期间，来源窗口临时增加 `WS_EX_NOACTIVATE`，宿主临时进入置顶窗口带，透视显示层则作为宿主拥有窗口保持在宿主上方。普通来源即使短暂激活也无法覆盖宿主；WinEvent 守卫仍监听前台切换与窗口重排，在来源开始越过宿主时立即钳制，并由独立后台工作以 2 ms 间隔短暂复查。快速钳制不进入 DWM/WGC 抓帧或 GPU 合成线程；会话结束恢复宿主原始置顶状态与显示层拥有关系。

PierceView does not synthesize mouse input. A small aperture following the pointer center lets Windows deliver real pointer events to the window beneath; the full circle or rectangle belongs only to the visual layer. The GPU visual window uses system-level layered transparent hit-test skipping, and the host redraws the old location after its aperture moves. During a session, sources temporarily gain `WS_EX_NOACTIVATE`, the host enters the topmost band, and the portal surface remains above it as an owned window. An ordinary source cannot cover the host even if it briefly activates. The WinEvent guard still watches foreground changes and window reordering, clamps a source as it starts crossing the host, and performs brief 2 ms follow-up checks on independent background work. The fast clamp never runs on the DWM/WGC capture or GPU composition thread; session teardown restores the host's original topmost state and portal ownership.
