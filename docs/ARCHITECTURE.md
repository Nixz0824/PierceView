...
圆形和圆角矩形都由分层位图 alpha 蒙版保证。`--visual-smoke` 会覆盖静止刷新、真实鼠标移动、后台 Hover 重绘、内容坐标对齐、稳定画布重定位次数、缓存即时提交、旧位置残留、提交前鼠标延迟锁定、换帧预算、羽化 alpha、黑帧和形状回归。个别来源窗口若 DWM 缩略图/PrintWindow 抓取失败，预览可能不可用。更激进的合成方式不在本页承诺范围内。

Both circles and rounded rectangles are guaranteed by layered bitmap alpha. `--visual-smoke` covers stationary refresh, real-pointer motion, background hover repaint, content alignment, stable-canvas relocation counts, immediate cached presentation, stale-position cleanup, pre-present pointer late-latching, frame-budget metrics, feather alpha, black frames, and shape regressions. Some source windows may fail DWM thumbnail/PrintWindow capture. More aggressive compositing approaches are outside this document’s promises.
