...
5. 预览在屏外窗维护单张带 64 像素安全边界的 DWM 缩略图。边界内移动只从复用缓冲中做 CPU 对齐裁剪，不改变 DWM 来源；越过边界才重定位缩略图。最终尺寸抓帧套用启用时预计算的形状 alpha，再用 `UpdateLayeredWindow` 一次提交内容和屏幕位置。Keep one off-screen DWM thumbnail with a 64 px safety margin. Motion inside the margin uses CPU-aligned cropping from a reused buffer without changing the DWM source; only crossing the margin repositions the thumbnail. Apply the precomputed shape alpha to the final-sized crop, then submit content and screen position together with one `UpdateLayeredWindow` call.
