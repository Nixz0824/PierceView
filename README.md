...
**公开版本 / Public release:** `1.0.6`　|　**本地 CPU 开发版本 / Local CPU development:** `2.1.0-cpu-alpha.2`
...
CPU 版本使用比透视形状四周各大 96 像素的稳定画布。鼠标在安全范围内移动时，原生显示窗口保持不动，只把上一张原始抓帧中的形状立即移到新位置；后台内容则继续约 60Hz 抓取，并在提交前读取最新鼠标坐标重新对齐。只有跨出安全范围才同步更新 DWM 来源和显示画布；鼠标中心的小型交互孔每次移动后会立即请求 Windows 重绘旧位置，以减少抖动、残影与旧路径闪现。

The CPU build uses a stable canvas with a 96 px margin around the portal. While the pointer stays inside that margin, the native display window does not move; the shape is immediately repositioned over the latest raw captured frame. Background content continues to capture at roughly 60Hz and is late-latched to the newest pointer coordinates before presentation. DWM source and display-canvas relocation happen only after crossing the margin; after each move, the small input aperture at the pointer explicitly asks Windows to repaint its old position, reducing jitter, ghosting, and stale-path flashes.
...
设置包含透视形状、圆形清晰区半径或矩形宽高、圆形/矩形共用的边缘羽化，以及界面语言；配置保存在 `%LOCALAPPDATA%\PierceView\settings.json`。Settings include portal shape, clear circle radius or rectangle dimensions, shared circle/rectangle edge feathering, and UI language; the configuration stays in that local file.
...
- 羽化可明显减轻圆形硬边的阶梯感；个别 GPU/驱动或来源窗口仍可能偶发短暂黑帧或闪烁。Feathering reduces hard-circle stair-stepping; some GPU/driver or source-window combinations may still briefly black-frame or flicker.
- 圆形使用径向 alpha 羽化，矩形沿自动圆角轮廓羽化；两者都不会模糊后台内容本身，设为 0 即恢复硬边。Circles use radial alpha feathering and rectangles follow their automatic rounded contour; neither blurs the background content itself, and 0 restores a hard edge.
